...
public class MainController {
    ...
    private void setupMainCalendar() {
        ...
        // Imposta l'altezza del contenitore per permettere lo scrolling sincronizzato
        timeLabelsContainer.setPrefHeight(24 * HOUR_HEIGHT + 20);

        for (int i = 0; i <= 24; i++) {
            double hourY = i * HOUR_HEIGHT;
            
            // Etichetta Ora Principale (es. 09:00)
            ...
            // Linea orizzontale principale
            Line line = new Line(0, hourY, 2000, hourY);
            line.getStyleClass().add("timeline-line-hour");
            calendarTimelineArea.getChildren().add(line);

            // Sottodivisioni (15, 30, 45 min)
            if (i < 24) {
                for (int q = 1; q < 4; q++) {
                    double qy = hourY + (q * 15 * MINUTE_HEIGHT);
                    
                    Label subLabel = new Label(String.format("%02d:%02d", i, q * 15));
                    subLabel.getStyleClass().add("sub-hour-label");
                    AnchorPane.setRightAnchor(subLabel, 10.0);
                    AnchorPane.setTopAnchor(subLabel, qy - 7);
                    timeLabelsContainer.getChildren().add(subLabel);

                    Line qLine = new Line(0, qy, 2000, qy);
                    qLine.getStyleClass().add("timeline-line");
